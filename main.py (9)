...
async def regen(user=Depends(get_current_user)):
    """AFK regen: 0.1 pixel per second since last call, capped."""
    if not user:
        raise HTTPException(401, "Login required")
    now = datetime.utcnow()
    last = user.get("last_regen") or user.get("created_at") or now
    ...
    # 0.1 per second
    gained_f = elapsed * 0.1
    # store fractional accumulator
    frac = float(user.get("pixel_frac") or 0) + gained_f
    whole = int(frac)
    frac = frac - whole
    limit = get_pixel_limit(user)
