...
async def place_pixel(data: PlacePixel, user=Depends(get_current_user)):
    ...
    await manager.broadcast({"type": "pixel", "x": x, "y": y, "color": color, "username": user["username"], "t": now.isoformat(), "war": True})
...
async def place_batch(data: PlaceBatch, user=Depends(get_current_user)):
    ...
    if placed:
        ...
        await manager.broadcast({"type": "pixels", "pixels": placed, "username": user["username"], "t": now.isoformat(), "war": True})
