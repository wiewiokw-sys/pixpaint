...
async def get_all_pixels(region: str = "all"):
    """Full map at client boot. Europe first (priority), then the rest. No chunks."""
    # Europe bbox on 4096 map (approx lon -10..40, lat 35..70)
    EU = (1800, 400, 2700, 1400)  # x1,y1,x2,y2
    eu_xs, eu_ys, eu_cs = [], [], []
    ot_xs, ot_ys, ot_cs = [], [], []
    if pixels_col is not None:
        try:
            for doc in pixels_col.find({}, {"_id": 0, "x": 1, "y": 1, "color": 1}).batch_size(5000):
                try:
                    x, y = int(doc["x"]), int(doc["y"])
                    c = doc["color"]
                except Exception:
                    continue
                if EU[0] <= x <= EU[2] and EU[1] <= y <= EU[3]:
                    eu_xs.append(x); eu_ys.append(y); eu_cs.append(c)
                else:
                    ot_xs.append(x); ot_ys.append(y); ot_cs.append(c)
            if region == "eu":
                return {"xs": eu_xs, "ys": eu_ys, "cs": eu_cs, "count": len(eu_xs), "region": "eu"}
            if region == "rest":
                return {"xs": ot_xs, "ys": ot_ys, "cs": ot_cs, "count": len(ot_xs), "region": "rest"}
            # default: Europe first, then rest
            return {
                "xs": eu_xs + ot_xs,
                "ys": eu_ys + ot_ys,
                "cs": eu_cs + ot_cs,
