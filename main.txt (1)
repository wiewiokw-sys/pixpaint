...
_recent_pixels: List[dict] = []  # [{x,y,color,username,t,ts}] TTL 60s

RECENT_PIXEL_TTL = 60  # seconds
# Anti-lag: per-user place rate limit (even god mode)
...
def _check_place_rate(username: str, n: int = 1) -> bool:
    """Return True if allowed. Always allows but may trim — caller uses returned allowed count."""
...
def _place_rate_room(username: str) -> int:
    now = datetime.utcnow().timestamp()
    arr = _place_rate.get(username) or []
    arr = [t for t in arr if now - t < PLACE_RATE_WINDOW]
    _place_rate[username] = arr
    return max(0, PLACE_RATE_MAX - len(arr))
...
async def place_pixel(data: PlacePixel, user=Depends(get_current_user)):
    ...
    if not _check_place_rate(user["username"], 1):
        raise HTTPException(429, "Too fast — slow down")
    ...
    # compact broadcast (no username every time — less lag)
    await manager.broadcast({"type": "pixel", "x": x, "y": y, "c": color, "war": 1})
...
async def place_batch(data: PlaceBatch, user=Depends(get_current_user)):
    ...
    room = _place_rate_room(user["username"])
    if room <= 0:
        raise HTTPException(429, "Too fast — slow down")
    pts = data.pixels[:min(200, room)]
    placed = []
    skipped = 0
    for p in pts:
    ...
    if placed:
        ...
        _check_place_rate(user["username"], len(placed))
        # compact: list of [x,y,c] — much smaller than JSON objects
        compact = [[p["x"], p["y"], p["color"]] for p in placed]
        await manager.broadcast({"type": "px", "p": compact, "war": 1})
...
async def get_radar(user=Depends(get_current_user)):
    """Recent pixels (last 60s) — auto-purged from RAM."""
    _prune_recent_pixels()
