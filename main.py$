...
async def lookup(x: int = Query(...), y: int = Query(...), user=Depends(get_current_user)):
    """Who placed the pixel at x,y — full profile for popup. Mod only."""
...
async def chat_history(channel: str = "en", limit: int = 25):
    ...
    cursor = chats_col.find({"channel": channel, "pinned": {"$ne": True}}).sort("created_at", DESCENDING).limit(min(limit, lim))
    msgs = list(cursor)[::-1]
    out = []
...
async def send_dm(data: dict = Body(...), user=Depends(get_current_user)):
    ...
    if chats_col is not None:
        chats_col.insert_one(doc)
        extra = list(chats_col.find({"channel": ch}).sort("created_at", DESCENDING).skip(50))
        if extra:
            chats_col.delete_many({"_id": {"$in": [e["_id"] for e in extra]}})
    await manager.broadcast({
