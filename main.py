...
def user_public(user: dict) -> dict:
    ...
    return {
        ...
        "medals": {
            ...
            "veteran": bool(user.get("veteran_medal")),
        },
...
async def place_pixel(data: PlacePixel, user=Depends(get_current_user)):
    ...
    # white on white: no cost, no store, no broadcast (invisible)
    if color in ("#ffffff", "#fff"):
        if pixels_col is not None:
            existing = pixels_col.find_one({"x": x, "y": y})
            if not existing:
                # treat as free invisible placement
                return {"ok": True, "pixels_left": user.get("pixels_left", 0), "skipped": True, "invisible": True}
    ...
    if pixels_col is not None:
        ...
        # upsert replaces previous owner info completely
        ...
        }, "$unset": {"prev_user": "", "history": ""}}, upsert=True)
...
# ---------- MOD: erase / ranking / accounts ----------
class ErasePixel(BaseModel):
    x: int
    y: int

@app.post("/mod/erase_pixel")
async def mod_erase_pixel(data: ErasePixel, user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    x, y = data.x, data.y
    if pixels_col is not None:
    ...
    cache_del_pixel(x, y)
    await manager.broadcast({"type": "pixel", "x": x, "y": y, "color": None, "erase": True, "username": user["username"]})
    return {"ok": True}

@app.post("/mod/set_pixels")
async def mod_set_pixels(data: dict = Body(...), user=Depends(get_current_user)):
    """Reduce or set placed pixels for ranking."""
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    target = (data.get("target") or "").strip().lstrip("@").lower()
    amount = int(data.get("amount") or 0)
    mode = (data.get("mode") or "set").lower()  # set | sub | zero
    t = users_col.find_one({"username": target})
    if not t:
        raise HTTPException(404, "User not found")
    if mode == "zero":
        users_col.update_one({"_id": t["_id"]}, {"$set": {"pixels_placed": 0, "pixels_placed_day": 0}})
    elif mode == "sub":
        users_col.update_one({"_id": t["_id"]}, {"$inc": {"pixels_placed": -abs(amount), "pixels_placed_day": -abs(amount)}})
        users_col.update_one({"_id": t["_id"]}, {"$max": {"pixels_placed": 0, "pixels_placed_day": 0}})
    else:
        users_col.update_one({"_id": t["_id"]}, {"$set": {"pixels_placed": max(0, amount)}})
    return {"ok": True}

@app.post("/mod/delete_account")
async def mod_delete_account(data: ModAction, user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
...
@app.post("/mod/unmute")
async def mod_unmute(data: ModAction, user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    target = (data.target or "").strip().lstrip("@").lower()
    users_col.update_one({"username": target}, {"$unset": {"muted_until": "", "muted_by": ""}, "$set": {"chat_spam": []}})
    return {"ok": True}

# ---------- EARN: animal / colors / math ----------
...
COLOR_NAMES = ["white","black","red","blue","orange","green","cyan","yellow","pink"]
COLOR_HEX = {
    "white":"#ffffff","black":"#000000","red":"#e50000","blue":"#0000ea",
    "orange":"#ff8000","green":"#02be01","cyan":"#00d3dd","yellow":"#fff200","pink":"#ff54b2"
}
...
async def earn_start(user=Depends(get_current_user)):
    ...
    limit = get_pixel_limit(user)
    # apply stok boost
    bst = user.get("boost_stok_until")
    if bst and isinstance(bst, datetime) and bst > datetime.utcnow():
        limit += int(user.get("boost_stok") or 0)
    cur = int(user.get("pixels_left") or 0)
    if cur >= limit:
        raise HTTPException(400, "Limit reached — spend pixels first")
    sid = str(random.randint(100000, 999999))
    mode = random.choice(["animal", "colors", "math"])
    if mode == "animal":
        ...
        payload = {"session_id": sid, "mode": "animal", "options": options, "animal": correct, "cooldown": 15, "theme": "green"}
    elif mode == "colors":
        correct = random.choice(COLOR_NAMES)
        opts = COLOR_NAMES[:]  # 9 colors
        random.shuffle(opts)
        earn_sessions[sid] = {"type": "colors", "correct": correct}
        payload = {"session_id": sid, "mode": "colors", "options": opts, "colors_hex": COLOR_HEX, "correct_name": correct, "cooldown": 15, "theme": "pink"}
    else:
        ...
        options = {ans}
        while len(options) < 9:
            options.add(ans + random.randint(-8, 8))
        options = list(options)
        random.shuffle(options)
        earn_sessions[sid] = {"type": "math", "correct": str(ans)}
        payload = {"session_id": sid, "mode": "math", "question": f"{a} {op} {b}", "options": [str(o) for o in options], "cooldown": 15, "theme": "red"}
...
async def earn_check(data: EarnCheck, session_id: str = Query(...), user=Depends(get_current_user)):
    ...
    # personal earn boost
    be = user.get("boost_earn_until")
    if be and isinstance(be, datetime) and be > datetime.utcnow():
        bonus += int(user.get("boost_earn") or 0)
...
async def _process_mod_command(raw: str, user: dict) -> Optional[dict]:
    ...
    # /abuse_night_800m_true  (night = ×1/5 on earn/afk/stock)
    if low.startswith("/abuse_night"):
        minutes = 55
        overlay = False
        mm = re.search(r"(\d+)m", low)
        if mm:
            minutes = max(1, min(int(mm.group(1)), 24 * 60 * 7))
        if low.endswith("_true") or "time_true" in low or "_true" in low:
            overlay = True
        _global_event = {
            "mode": "night",
        ...
        return {"ok": True, "silent": True, "msg": f"Night event {minutes}m overlay={overlay} (×1/5)"}

    # /abuse_@user_AFK/EARN/STOK_qty_minutes
    m_abuse = re.match(r"^/abuse_@?([a-z0-9_]{2,24})_(afk|earn|stok)_(\d+)_(\d+)m?$", low)
    if m_abuse:
        target, kind, qty, minutes = m_abuse.group(1), m_abuse.group(2), int(m_abuse.group(3)), int(m_abuse.group(4))
        minutes = max(1, min(minutes, 24 * 60 * 7))
        qty = max(0, min(qty, 100000))
        t = users_col.find_one({"username": target})
        if not t:
            return {"ok": True, "silent": True, "msg": "User not found"}
        until = datetime.utcnow() + timedelta(minutes=minutes)
        if kind == "afk":
            users_col.update_one({"_id": t["_id"]}, {"$set": {"boost_afk": qty, "boost_afk_until": until}})
        elif kind == "earn":
            users_col.update_one({"_id": t["_id"]}, {"$set": {"boost_earn": qty, "boost_earn_until": until}})
        elif kind == "stok":
            users_col.update_one({"_id": t["_id"]}, {"$set": {"boost_stok": qty, "boost_stok_until": until}})
        return {"ok": True, "silent": True, "msg": f"Boost {kind} +{qty} for @{target} {minutes}m"}
    ...
    if low.startswith("/abuse_won"):
        minutes = 55
        overlay = False
        opacity = 90  # percent visible for overlay color fog
        mm = re.search(r"(\d+)m", low)
        if mm:
            minutes = max(1, min(int(mm.group(1)), 24 * 60))
        if "_true" in low or "time_true" in low:
            overlay = True
        opm = re.search(r"_(\d{1,2})(?:%|)$", low.replace("_true",""))
        if not opm:
            opm = re.search(r"_true_(\d{1,2})", low)
        if opm:
            opacity = max(1, min(int(opm.group(1)), 99))
        _global_event = {
            "mode": "won",
            "ends_at": datetime.utcnow() + timedelta(minutes=minutes),
            "overlay": overlay,
        ...
        await manager.broadcast({"type": "event", "mode": "won", "minutes": minutes, "overlay": overlay, "opacity": opacity, "by": user["username"]})
        return {"ok": True, "silent": True, "msg": f"Won event {minutes}m opacity={opacity}%"}
    ...
    # /window_sms(text)_seconds  or  /window_sms text_4
    if low.startswith("/window_sms"):
        raw = msg[len("/window_sms"):].strip()
        duration = 0
        text = raw
        # (text)_4
        mpar = re.match(r"^\((.+)\)_(\d+)$", raw)
        if mpar:
            text, duration = mpar.group(1), int(mpar.group(2))
        else:
            m2 = re.match(r"^(.+)_(\d+)$", raw)
            if m2:
                text, duration = m2.group(1).strip(), int(m2.group(2))
        text = (text or "").strip()
        if not text:
            return {"ok": True, "silent": True, "msg": "Usage: /window_sms(text)_seconds"}
        duration = max(0, min(duration, 120))
        _window_sms = {
            "text": text[:400],
            "from": user["username"],
            "display_name": user.get("display_name") or user["username"],
        ...
        return {"ok": True, "silent": True, "msg": f"Window SMS sent ({duration}s)"}
...
async def send_chat(message: str = Body(..., embed=True), channel: str = Body("en"), user=Depends(get_current_user)):
    ...
    # auto spam mute: 3 same messages → 3 min; then 2 more same → 30 min
    msg_norm = (message or "").strip().lower()
    spam = list(user.get("chat_spam") or [])
    spam.append(msg_norm)
    spam = spam[-5:]
    users_col.update_one({"_id": user["_id"]}, {"$set": {"chat_spam": spam}})
    if len(spam) >= 3 and spam[-1] == spam[-2] == spam[-3]:
        # check if already had a 3-min mute for this spam streak
        if spam.count(msg_norm) >= 5:
            until = datetime.utcnow() + timedelta(minutes=30)
            users_col.update_one({"_id": user["_id"]}, {"$set": {"muted_until": until, "muted_by": "auto-spam"}})
            raise HTTPException(403, "MUTED_BY:@auto-spam|MIN:30")
        elif spam.count(msg_norm) >= 3:
            until = datetime.utcnow() + timedelta(minutes=3)
            users_col.update_one({"_id": user["_id"]}, {"$set": {"muted_until": until, "muted_by": "auto-spam"}})
            raise HTTPException(403, "MUTED_BY:@auto-spam|MIN:3")
    if channel not in ("global", "en", "ru", "clan"):
        channel = "en"
    doc = {
        "username": user["username"],
        "display_name": user.get("display_name") or user["username"],
    ...
    if chats_col is not None:
        chats_col.insert_one(doc)
        keep = 30 if channel == "clan" else 30
...
# ---- QUESTIONS / ASK DEV ----
questions_col = None  # bound in lifespan if needed

@app.get("/questions")
async def list_questions(user=Depends(get_current_user)):
    global questions_col
    if news_col is not None and questions_col is None:
        questions_col = db["questions"] if db is not None else None
    if questions_col is None:
        return []
    q = {}
    if not (user and user.get("is_mod")):
        q = {"$or": [{"published": True}, {"username": (user or {}).get("username")}]}
    out = []
    for doc in questions_col.find(q).sort("created", -1).limit(50):
        out.append({
            "id": str(doc["_id"]),
            "text": doc.get("text"),
...
async def post_question(data: dict = Body(...), user=Depends(get_current_user)):
    ...
    if questions_col is None:
        raise HTTPException(503)
    text = (data.get("text") or "")[:500].strip()
    if not text:
...
async def like_question(data: dict = Body(...), user=Depends(get_current_user)):
    global questions_col
    if not user:
        raise HTTPException(401)
    if questions_col is None and db is not None:
        questions_col = db["questions"]
    if questions_col is None:
        raise HTTPException(503)
    try:
        oid = ObjectId(data.get("qid"))
    except Exception:
        raise HTTPException(400)
    ...
    if un in liked:
        ...
        liked = liked + [un]
    questions_col.update_one({"_id": oid}, {"$set": {"liked_by": liked, "likes": len(liked)}})
    return {"ok": True, "likes": len(liked)}
...
async def answer_question(data: dict = Body(...), user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(400)
    ans = (data.get("answer") or "")[:1000]
    questions_col.update_one({"_id": oid}, {"$set": {"answer": ans, "answered_by": user["username"]}})
    return {"ok": True}

@app.post("/questions/publish")
async def publish_question(data: dict = Body(...), user=Depends(get_current_user)):
    global questions_col
    if not user or not user.get("is_mod"):
        raise HTTPException(403)
    if questions_col is None and db is not None:
    ...
    try:
        ...
        raise HTTPException(400)
    questions_col.update_one({"_id": oid}, {"$set": {"published": True}})
    return {"ok": True}

@app.post("/mod/grant_veteran")
async def grant_veteran(data: ModAction, user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    target = (data.target or "").strip().lstrip("@").lower()
    users_col.update_one({"username": target}, {"$set": {"veteran_medal": True}})
    return {"ok": True}

@app.post("/mod/erase_user_pixels")
async def erase_user_pixels(data: ModAction, user=Depends(get_current_user)):
    """Erase all pixel ownership info for user + zero placed counters."""
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    target = (data.target or "").strip().lstrip("@").lower()
    t = users_col.find_one({"username": target})
    if not t:
        raise HTTPException(404)
    if pixels_col is not None:
        # remove pixels placed by this user
        res = pixels_col.delete_many({"username": target})
        # bust cache partially - full rebuild next load
        global _pixel_cache_ready
        _pixel_cache_ready = False
