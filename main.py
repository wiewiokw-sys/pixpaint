...
@app.get("/canvas/all")
async def get_all_pixels(skip: int = 0, limit: int = 15000):
    """Paged pixels from Mongo (no full RAM dump). Client loops until count < limit."""
    lim = max(1, min(int(limit or 15000), 25000))
    sk = max(0, int(skip or 0))
    xs, ys, cs = [], [], []
    if pixels_col is None:
        return {"xs": xs, "ys": ys, "cs": cs, "count": 0, "skip": sk, "done": True}
    try:
        cur = (
            pixels_col.find({}, {"_id": 0, "x": 1, "y": 1, "color": 1, "c": 1})
            .skip(sk)
            .limit(lim)
            .batch_size(5000)
        )
        ...
        print("canvas/all err", e)
        return {"xs": [], "ys": [], "cs": [], "count": 0, "skip": sk, "done": True, "error": str(e)}
    return {
        "xs": xs,
        "ys": ys,
        "cs": cs,
