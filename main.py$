...
def user_public(user: dict) -> dict:
    ...
    try:
        ...
        if cid and clans_col is not None:
            clan = clans_col.find_one({"_id": ObjectId(cid)})
            if clan and clan.get("leader") == user.get("username") and len(clan.get("members") or []) >= 20:
                is_king = True
    except Exception:
        pass
    return {
...
async def register(data: RegisterBody):
    ...
    # 1 account per device per week
    did = (data.device_id or "").strip()[:64]
    if did:
        week_ago = datetime.utcnow() - timedelta(days=7)
        prev = users_col.find_one({"device_id": did, "created_at": {"$gte": week_ago}})
        if prev:
            raise HTTPException(400, "Only 1 account per device per week")
...
async def lookup(x: int = Query(...), y: int = Query(...)):
    """Who placed this pixel (from DB or recent cache)."""
    if not (0 <= x < CANVAS_SIZE and 0 <= y < CANVAS_SIZE):
        return {"empty": True}
    # recent first (2 min)
    for r in reversed(_recent_pixels):
        if r.get("x") == x and r.get("y") == y:
            return {"x": x, "y": y, "color": r.get("color"), "username": r.get("username"),
                    "display_name": r.get("username"), "t": r.get("t"), "recent": True}
    if pixels_col is not None:
        try:
            doc = pixels_col.find_one({"x": x, "y": y}, {"_id": 0})
            if doc:
                return {"x": x, "y": y, "color": doc.get("color"), "username": doc.get("username"),
                        "display_name": doc.get("display_name") or doc.get("username"),
                        "t": str(doc.get("placed_at") or ""), "recent": False}
        except Exception as e:
            print("lookup err", e)
    return {"empty": True}
...
async def publish_template(data: PublicTemplate, user=Depends(get_current_user)):
    ...
    if existing >= 2 and not user.get("is_mod"):
        raise HTTPException(400, "Max 2 public templates")
...
async def send_chat(message: str = Body(..., embed=True), channel: str = Body("en"), user=Depends(get_current_user)):
    ...
    if message.startswith("'") and len(message) > 1:
        ...
        if last_pin and isinstance(last_pin, datetime) and (datetime.utcnow() - last_pin).total_seconds() < 20 * 60 and not user.get("is_mod"):
            raise HTTPException(400, "Pin once per 20 minutes")
        pinned = True
        pin_until = datetime.utcnow() + timedelta(hours=2)
        message = message[1:].lstrip()[:200]
    ...
    doc = {
        ...
        "is_mod": user.get("is_mod", False),
        "avatar_color": user.get("avatar_color", 0),
        "name_color": user.get("name_color", user.get("avatar_color", 0)),
        "created_at": datetime.utcnow(),
        "pinned": pinned,
        "pin_until": pin_until,
    }
    ...
    await manager.broadcast({"type": "chat", "username": doc["username"], "display_name": doc["display_name"],
        ...
        "pinned": pinned, "created_at": str(doc["created_at"])})
    return {"ok": True}
...
async def recent_users(user=Depends(get_current_user)):
    online = []
    seen = set()
    for info in online_users.values():
        un = info.get("username") or "guest"
        if un in seen or un == "guest":
            continue
        seen.add(un)
        online.append({"username": un, "display_name": info.get("display_name") or un, "online": True, "last_pixel": None})
    if not user or not user.get("is_mod"):
        return {"online": online, "recent": []}
    recent = []
    if users_col is not None:
        for u in users_col.find({"last_seen": {"$ne": None}}).sort("last_seen", DESCENDING).limit(30):
            un = u.get("username")
            if not un: continue
            recent.append({
                "username": un, "display_name": u.get("display_name") or un,
                "online": un in seen, "last_seen": str(u.get("last_seen") or ""),
                "last_pixel": u.get("last_pixel"),
            })
...
@app.post("/mod/delete")
async def mod_delete(data: dict = Body(...), user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    ...
    try:
        ...
        raise HTTPException(400, "Bad id")
    if kind == "news" and news_col is not None:
        news_col.delete_one({"_id": _id})
    elif kind == "question":
        _ensure_extra_cols()
        if questions_col is not None: questions_col.delete_one({"_id": _id})
    elif kind == "report":
        _ensure_extra_cols()
        if reports_col is not None: reports_col.delete_one({"_id": _id})
    elif kind == "template" and public_tpl_col is not None:
        public_tpl_col.delete_one({"_id": _id})
    elif kind == "chat" and chats_col is not None:
        chats_col.delete_one({"_id": _id})
    else:
