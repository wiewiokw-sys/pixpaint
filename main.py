...
async def get_chunk(x1: int = 0, y1: int = 0, x2: int = 256, y2: int = 256):
...
def _find_file(name: str) -> Optional[str]:
    """Шукає файл поруч з main.py / у cwd / у /opt/render/..."""
    candidates = [
        name,
...
async def root():
    ...
    return HTMLResponse(
        ...
        "<p>Upload <b>index.html</b> and <b>world_map.png</b> next to <b>main.py</b> on Render.</p>"
        "<p>Files in project root: main.py, index.html, world_map.png, requirements.txt</p>",
        status_code=404,
    )
