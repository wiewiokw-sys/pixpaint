...
DELETE_COLOR = "#fe00fe"  # moderator eraser — deletes any pixel under it
...
# Global server events: mode=None|night|haos|won, ends_at, overlay bool
_global_event: Dict[str, Any] = {"mode": None, "ends_at": None, "overlay": False, "set_by": None}
...
def get_global_event() -> dict:
    ...
    if ends and isinstance(ends, datetime) and datetime.utcnow() >= ends:
        _global_event = {"mode": None, "ends_at": None, "overlay": False, "set_by": None}
    return dict(_global_event)

def event_afk_multiplier() -> float:
    m = get_global_event().get("mode")
    if m == "night":
        return 1 / 3
    if m == "haos":
        return 2.0
...
def get_pixel_limit(user) -> int:
    ...
    # global event multiplier (night / haos / won)
    ev = get_global_event()
...
def normalize_username(raw: str) -> str:
    """Only Latin a-z, digits, underscore. No Cyrillic or other scripts."""
    un = (raw or "").strip().lstrip("@").lower()
    un = re.sub(r"[^a-z0-9_]", "", un)
    return un

def validate_username(un: str) -> str:
    un = normalize_username(un)
    if len(un) < 3 or len(un) > 20:
        raise HTTPException(400, "Username 3-20 chars: Latin a-z, 0-9, _ only")
    if not re.match(r"^[a-z][a-z0-9_]*$", un):
        raise HTTPException(400, "Username must start with a letter (Latin only)")
    return un
...
async def register(data: RegisterBody):
    ...
    uname = validate_username(data.username)
...
async def profile_update(data: UpdateProfile, user=Depends(get_current_user)):
    ...
    if data.username is not None:
        un = validate_username(data.username)
...
async def place_pixel(data: PlacePixel, user=Depends(get_current_user)):
    ...
    is_eraser = color == DELETE_COLOR
    if is_eraser and not user.get("is_mod") and not user.get("god_mode"):
        raise HTTPException(403, "Eraser is mod-only")
    zone = check_zone(x, y)
    if zone and not is_eraser:
        raise HTTPException(403, f"Protected by @{zone.get('mod','mod')} until {zone.get('expires_at')}")
    # same color = free (not for eraser)
    if pixels_col is not None and not is_eraser:
    ...
    now = datetime.utcnow()
    if is_eraser:
        # delete pixel under eraser
        if pixels_col is not None:
            pixels_col.delete_one({"x": x, "y": y})
        cache_del_pixel(x, y)
        users_col.update_one({"_id": user["_id"]}, {"$set": {
            "last_seen": now,
            "last_pixel": {"x": x, "y": y, "color": "erase", "t": now.isoformat()}
        }})
        last_remote_pixel = {"x": x, "y": y, "color": None, "username": user["username"], "t": now.isoformat(), "erase": True}
        await manager.broadcast({"type": "pixel_del", "x": x, "y": y, "username": user["username"], "t": now.isoformat()})
        left = user.get("pixels_left", 1) - (0 if user.get("god_mode") else 1)
        return {"ok": True, "pixels_left": max(0, left), "erased": True}
    if pixels_col is not None:
...
async def place_batch(data: PlaceBatch, user=Depends(get_current_user)):
    ...
    for p in pts:
        ...
        if zone and not is_eraser:
            continue
        if not is_eraser and pixels_col is not None:
            existing = pixels_col.find_one({"x": x, "y": y})
            if existing and norm_color(existing.get("color", "")) == color:
                skipped += 1
                continue
        if not user.get("god_mode") and user.get("pixels_left", 0) <= len(placed) + len(erased):
            break
        if is_eraser:
            if pixels_col is not None:
                pixels_col.delete_one({"x": x, "y": y})
            cache_del_pixel(x, y)
            erased.append({"x": x, "y": y})
            continue
        if pixels_col is not None:
            pixels_col.update_one({"x": x, "y": y}, {"$set": {
                "color": color, "user_id": str(user["_id"]),
    ...
    if placed or erased:
        total_ops = len(placed) + len(erased)
        inc = total_ops if not user.get("god_mode") else 0
        users_col.update_one({"_id": user["_id"]}, {
            "$inc": {"pixels_left": -inc, "pixels_placed": total_ops, "pixels_placed_day": total_ops},
            "$set": {"day_key": day_key()}
        })
        global last_remote_pixel
        now = datetime.utcnow()
    ...
    if erased:
        for e in erased:
            await manager.broadcast({"type": "pixel_del", "x": e["x"], "y": e["y"], "username": user["username"]})
    total_ops = len(placed) + len(erased)
    left = user.get("pixels_left", 0) - (0 if user.get("god_mode") else total_ops)
    return {"ok": True, "placed": len(placed), "erased": len(erased), "skipped": skipped, "pixels_left": left}
...
# Skip full RAM cache when map is large (Render free ~512MB OOM)
_PIXEL_CACHE_MAX = int(os.getenv("PIXEL_CACHE_MAX", "80000"))
_pixel_cache_disabled = False

def ensure_pixel_cache():
    """Load pixels into RAM once — skipped if too large (avoids 512MB OOM on Render)."""
    global _pixel_cache, _pixel_cache_ready, _pixel_cache_disabled
    if _pixel_cache_disabled:
        return
    ...
    if pixels_col is not None:
        try:
            try:
                est = pixels_col.estimated_document_count()
            except Exception:
                est = 0
            if est > _PIXEL_CACHE_MAX:
                print("Pixel cache DISABLED (too many pixels:", est, "> max", _PIXEL_CACHE_MAX, ")")
                _pixel_cache = None
                _pixel_cache_disabled = True
                _pixel_cache_ready = True
                return
            for doc in pixels_col.find({}, {"_id": 0, "x": 1, "y": 1, "color": 1}).batch_size(3000):
                try:
                    _pixel_cache[_cache_key(int(doc["x"]), int(doc["y"]))] = doc["color"]
                except Exception:
                    pass
                if len(_pixel_cache) > _PIXEL_CACHE_MAX:
                    print("Pixel cache aborted mid-load, switching to Mongo-only")
                    _pixel_cache = None
                    _pixel_cache_disabled = True
                    _pixel_cache_ready = True
                    return
...
async def get_chunk(x1: int = 0, y1: int = 0, x2: int = 512, y2: int = 512):
    """Compact packed format — much smaller JSON than list of {x,y,color}."""
    x1, y1 = max(0, int(x1)), max(0, int(y1))
    # hard cap chunk size so one request cannot scan half the map
    if x2 - x1 > 512:
        x2 = x1 + 512
    if y2 - y1 > 512:
        y2 = y1 + 512
        ...
        try:
            ...
            for doc in cur:
                try:
                    xs.append(int(doc["x"]))
                    ys.append(int(doc["y"]))
                    cs.append(doc.get("color") or doc.get("c") or "#000")
                except Exception:
                    pass
            return {"xs": xs, "ys": ys, "cs": cs, "n": len(xs)}
        except Exception as e:
    ...
    if _pixel_cache:
        for k, col in _pixel_cache.items():
            try:
                ...
                continue
            if x1 <= x < x2 and y1 <= y < y2:
                xs.append(x); ys.append(y); cs.append(col)
    return {"xs": xs, "ys": ys, "cs": cs, "n": len(xs)}

@app.get("/canvas/sample")
async def canvas_sample(limit: int = 50):
    """Fast debug: first N pixels from DB (any region)."""
    xs, ys, cs = [], [], []
    if pixels_col is None:
        return {"xs": xs, "ys": ys, "cs": cs, "count": 0, "total_est": 0}
    lim = max(1, min(int(limit), 500))
    try:
        total = pixels_col.estimated_document_count()
    except Exception:
        total = -1
    try:
        for doc in pixels_col.find({}, {"_id": 0, "x": 1, "y": 1, "color": 1, "c": 1}).limit(lim):
            try:
                xs.append(int(doc["x"]))
                ys.append(int(doc["y"]))
                cs.append(doc.get("color") or doc.get("c") or "#000000")
            except Exception:
                pass
    except Exception as e:
        print("sample err", e)
    return {"xs": xs, "ys": ys, "cs": cs, "count": len(xs), "total_est": total}

@app.get("/canvas/all")
async def get_all_pixels(region: str = "all"):
    """Full map at client boot. Europe first, then rest. Uses bbox queries (fast)."""
    # Europe bbox on 4096 map (approx lon -10..40, lat 35..70)
    EU_Q = {"x": {"$gte": 1800, "$lte": 2700}, "y": {"$gte": 400, "$lte": 1400}}
    REST_Q = {"$or": [
        {"x": {"$lt": 1800}}, {"x": {"$gt": 2700}},
        {"y": {"$lt": 400}}, {"y": {"$gt": 1400}},
    ]}
    proj = {"_id": 0, "x": 1, "y": 1, "color": 1}
    ...
    def pack(query):
        ...
        if pixels_col is None:
            return xs, ys, cs
        try:
            for doc in pixels_col.find(query, proj).batch_size(5000):
                try:
                    xs.append(int(doc["x"]))
                    ys.append(int(doc["y"]))
                    cs.append(doc.get("color") or doc.get("c") or "#000")
            ...
            print("canvas/all mongo err", e)
        return xs, ys, cs

    if region == "eu":
        xs, ys, cs = pack(EU_Q)
        return {"xs": xs, "ys": ys, "cs": cs, "count": len(xs), "region": "eu"}
    if region == "rest":
        xs, ys, cs = pack(REST_Q)
        return {"xs": xs, "ys": ys, "cs": cs, "count": len(xs), "region": "rest"}
    # all: europe then rest
    eu_xs, eu_ys, eu_cs = pack(EU_Q)
    ot_xs, ot_ys, ot_cs = pack(REST_Q)
    return {
        "xs": eu_xs + ot_xs,
        "ys": eu_ys + ot_ys,
        "cs": eu_cs + ot_cs,
        "count": len(eu_xs) + len(ot_xs),
        "eu_count": len(eu_xs),
        "rest_count": len(ot_xs),
    }
...
EARN_COLORS = ["#000000","#ffffff","#e50000","#ff2323","#02be01","#0000ea","#ffca00","#820080","#50a3d7","#ff54b2","#fc9a29","#4e4e4e","#00d3dd","#940000","#6e4bed"]
...
async def earn_start(user=Depends(get_current_user)):
    ...
    if round_i == 0:
        ...
        if len(earn_sessions) > 20000:
            earn_sessions.clear()
        return {"session_id": sid, "mode": "animal", "options": options, "animal": correct, "cooldown": 15, "round": 1}
    if round_i == 1:
        correct = random.choice(EARN_COLORS)
        opts = random.sample([c for c in EARN_COLORS if c != correct], 8) + [correct]
        random.shuffle(opts)
        earn_sessions[sid] = {"type": "color", "correct": correct}
        if len(earn_sessions) > 20000:
            earn_sessions.clear()
        return {"session_id": sid, "mode": "color", "options": opts, "color": correct, "cooldown": 15, "round": 2}
    # math grade-4 style
    if streak > 20 and random.random() < 0.63:
    ...
    wrong = set()
    while len(wrong) < 8:
        w = correct_n + random.randint(-12, 12)
        if w != correct_n and 0 <= w <= 99:
            wrong.add(str(w))
    options = list(wrong) + [correct]
    random.shuffle(options)
    earn_sessions[sid] = {"type": "math", "correct": correct}
    if len(earn_sessions) > 20000:
        earn_sessions.clear()
    return {"session_id": sid, "mode": "math", "options": options, "question": q, "cooldown": 15, "round": 3}
...
async def earn_check(data: EarnCheck, session_id: str = Query(...), user=Depends(get_current_user)):
    ...
    add = (base + bonus) * event_earn_multiplier()
    if user.get("is_twink"):
        add = add / 3
    add = int(max(0, add))
    limit = get_pixel_limit(user)
...
@app.post("/templates/public/delete")
async def delete_public_template(tid: str = Body(..., embed=True), user=Depends(get_current_user)):
    ...
    if public_tpl_col is None:
        raise HTTPException(503)
    try:
        public_tpl_col.delete_one({"_id": ObjectId(tid)})
    except Exception:
...
async def list_videos():
    ...
    col = db["videos"]
    out = []
    for v in col.find().sort("created", -1).limit(50):
        out.append({
            "id": str(v["_id"]), "title": v.get("title"), "desc": v.get("desc"), "url": v.get("url"),
            "author": v.get("author"), "created": str(v.get("created") or ""),
            "likes": len(v.get("likes") or []),
        })
    return out

@app.post("/videos")
async def post_video(title: str = Body(...), url: str = Body(...), desc: str = Body(""), user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    if db is None:
        raise HTTPException(503)
    db["videos"].insert_one({
        "title": (title or "")[:80], "url": (url or "")[:500], "desc": (desc or "")[:400],
        "author": user["username"], "created": datetime.utcnow(), "likes": [],
    })
    return {"ok": True}

@app.post("/videos/delete")
async def delete_video(vid: str = Body(..., embed=True), user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    if db is None:
        raise HTTPException(503)
    try:
        db["videos"].delete_one({"_id": ObjectId(vid)})
    except Exception:
        raise HTTPException(400, "Bad id")
    return {"ok": True}

@app.post("/videos/like")
async def like_video(vid: str = Body(..., embed=True), user=Depends(get_current_user)):
    if not user:
        raise HTTPException(401)
    if db is None:
        raise HTTPException(503)
    try:
        ...
        likes = set(v.get("likes") or [])
        un = user["username"]
        if un in likes:
            likes.discard(un)
        else:
            likes.add(un)
        db["videos"].update_one({"_id": v["_id"]}, {"$set": {"likes": list(likes)}})
        return {"ok": True, "likes": len(likes)}
...
async def list_questions(user=Depends(get_current_user)):
    ...
    col = db["questions"]
    out = []
    for q in col.find().sort("created", -1).limit(80):
        # published for all; unpublished only for author + mods
        pub = bool(q.get("published"))
        is_own = user and q.get("username") == user.get("username")
        is_mod = user and user.get("is_mod")
        if not pub and not is_own and not is_mod:
            continue
        out.append({
            "id": str(q["_id"]), "text": q.get("text"), "username": q.get("username"),
            "display_name": q.get("display_name"), "answer": q.get("answer"),
            "answer_by": q.get("answer_by"), "answer_name": q.get("answer_name"),
            "published": pub, "likes": len(q.get("likes") or []),
            "created": str(q.get("created") or ""),
        })
    return out

@app.post("/questions")
async def ask_question(text: str = Body(..., embed=True), user=Depends(get_current_user)):
    if not user:
        raise HTTPException(401)
    if db is None:
        raise HTTPException(503)
    ...
    db["questions"].insert_one({
        ...
        "answer": None, "answer_by": None, "answer_name": None,
        "published": False, "likes": [], "created": datetime.utcnow(),
    })
    return {"ok": True}

@app.post("/questions/answer")
async def answer_question(qid: str = Body(...), answer: str = Body(...), user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    if db is None:
        raise HTTPException(503)
    try:
        db["questions"].update_one({"_id": ObjectId(qid)}, {"$set": {
            "answer": (answer or "")[:800],
            "answer_by": user["username"],
            "answer_name": user.get("display_name") or user["username"],
        }})
    except Exception:
        raise HTTPException(400, "Bad id")
    return {"ok": True}

@app.post("/questions/publish")
async def publish_question(qid: str = Body(..., embed=True), user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    if db is None:
        raise HTTPException(503)
    try:
        db["questions"].update_one({"_id": ObjectId(qid)}, {"$set": {"published": True}})
    except Exception:
        raise HTTPException(400, "Bad id")
    return {"ok": True}

@app.post("/questions/delete")
async def delete_question(qid: str = Body(..., embed=True), user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    if db is None:
        raise HTTPException(503)
...
@app.post("/questions/like")
async def like_question(qid: str = Body(..., embed=True), user=Depends(get_current_user)):
    if not user:
        raise HTTPException(401)
    if db is None:
        raise HTTPException(503)
    try:
        ...
        if not q:
            raise HTTPException(404)
        likes = set(q.get("likes") or [])
        un = user["username"]
        if un in likes:
            likes.discard(un)
        else:
            likes.add(un)
        db["questions"].update_one({"_id": q["_id"]}, {"$set": {"likes": list(likes)}})
        return {"ok": True, "likes": len(likes)}
    except HTTPException:
        raise
    except Exception:
...
async def clan_search(tag: str = Query(...)):
    raw = (tag or "").strip()
    # "@" or empty → list all clans
    if raw in ("", "@"):
        if clans_col is None:
            return []
        out = []
        for c in clans_col.find({}, {"name": 1, "tag": 1, "leader": 1, "members": 1}).limit(100):
...
async def _process_mod_command(raw: str, user: dict) -> Optional[dict]:
    ...
    # /windows_sms(text) or /window_sms(text)
    m_ws = re.match(r"^/windows?_sms\(([\s\S]*)\)$", msg, re.I)
    if m_ws or low.startswith("/window_sms") or low.startswith("/windows_sms"):
        if m_ws:
            text = m_ws.group(1).strip()
        else:
            text = msg.split("(", 1)[-1].rstrip(")").strip() if "(" in msg else msg.split(" ", 1)[-1].strip()
            if low in ("/window_sms", "/windows_sms"):
                text = ""
        if not text:
            return {"ok": True, "silent": True, "msg": "Usage: /windows_sms(your text)"}
        _window_sms = {
            "text": text[:400],
            "from": user["username"],
            "display_name": user.get("display_name") or user["username"],
...
async def send_chat(message: str = Body(..., embed=True), channel: str = Body("en"), user=Depends(get_current_user)):
    ...
    # /add_@username — open/create DM (everyone)
    m_add = re.match(r"^/add_@?([a-z0-9_]{2,20})$", (message or "").strip(), re.I)
    if m_add:
        target = m_add.group(1).lower()
        if target == user["username"]:
            raise HTTPException(400, "Cannot DM yourself")
        if users_col.find_one({"username": target}) is None:
            raise HTTPException(404, "User not found")
        a, b = sorted([user["username"], target])
        dm_ch = f"dm:{a}:{b}"
        return {"ok": True, "silent": True, "dm": dm_ch, "msg": f"DM @{target}"}
    # mod silent commands
    ...
    # allow dm:user:user channels
    is_dm = isinstance(channel, str) and channel.startswith("dm:")
    if not is_dm and channel not in ("global", "en", "ru", "ar", "tr", "clan"):
        channel = "en"
    if is_dm:
        parts = channel.split(":")
        if len(parts) != 3 or user["username"] not in (parts[1], parts[2]):
            raise HTTPException(403, "Not your DM")
    doc = {
        "username": user["username"],
        "display_name": user.get("display_name") or user["username"],
        "message": message[:200],
    ...
    payload = {"type": "chat", "username": doc["username"], "display_name": doc["display_name"],
        "message": doc["message"], "channel": channel, "is_mod": doc["is_mod"],
        "avatar_color": doc["avatar_color"], "created_at": str(doc["created_at"]), "id": inserted_id}
    await manager.broadcast(payload)
    return {"ok": True, "id": inserted_id}

@app.post("/chat/delete")
async def chat_delete(mid: str = Body(..., embed=True), user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    ...
    try:
        ...
        raise HTTPException(400, "Bad id")
    await manager.broadcast({"type": "chat_delete", "id": mid})
    return {"ok": True}
...
async def chat_history(channel: str = "en", limit: int = 100):
    ...
    return [{"id": str(c["_id"]), "username": c["username"], "display_name": c.get("display_name"), "message": c["message"],
...
@app.post("/mod/rename")
async def mod_rename(
    target: str = Body(...),
    new_username: str = Body(...),
    user=Depends(get_current_user),
):
    """Moderator: force-change another player's @username (e.g. fix Cyrillic)."""
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    if users_col is None:
        raise HTTPException(503)
    old = target.strip().lstrip("@").lower()
    # allow finding users that still have non-latin chars stored
    victim = users_col.find_one({"username": old})
    if not victim:
        # try exact match without lower for legacy Cyrillic
        victim = users_col.find_one({"username": target.strip().lstrip("@")})
    if not victim:
        raise HTTPException(404, "User not found")
    new_un = validate_username(new_username)
    if new_un != victim["username"] and users_col.find_one({"username": new_un}):
        raise HTTPException(400, "Username taken")
    old_username = victim["username"]
    old_display = victim.get("display_name") or old_username
    users_col.update_one({"_id": victim["_id"]}, {"$set": {"username": new_un}})
    # cascade same as profile update
    try:
        name_set = {"username": new_un, "display_name": old_display}
        if pixels_col is not None:
            pixels_col.update_many(
                {"$or": [{"username": old_username}, {"user_id": str(victim["_id"])}]},
                {"$set": name_set},
            )
        if chats_col is not None:
            chats_col.update_many(
                {"username": old_username},
                {"$set": {"username": new_un, "display_name": old_display}},
            )
        if public_tpl_col is not None:
            public_tpl_col.update_many(
                {"author": old_username},
                {"$set": {"author": new_un, "author_name": old_display}},
            )
        if news_col is not None:
            news_col.update_many({"author": old_username}, {"$set": {"author": new_un}})
            for n in news_col.find({"comments.username": old_username}):
                comments = n.get("comments") or []
                for c in comments:
                    if c.get("username") == old_username:
        ...
        if clans_col is not None:
            for clan in clans_col.find({"$or": [{"leader": old_username}, {"members": old_username}]}):
                members = [new_un if m == old_username else m for m in (clan.get("members") or [])]
                leader = new_un if clan.get("leader") == old_username else clan.get("leader")
                clans_col.update_one({"_id": clan["_id"]}, {"$set": {"members": members, "leader": leader}})
    except Exception as e:
        print("mod rename cascade:", e)
    return {"ok": True, "old": old_username, "new": new_un}

@app.post("/device/register")
async def device_register(device_id: str = Body(...), label: str = Body(""), user=Depends(get_current_user)):
    if not user:
        raise HTTPException(401)
    did = (device_id or "")[:80]
    if not did:
        raise HTTPException(400, "device_id required")
    lab = (label or "device")[:40]
    ...
    for d in devices:
        if d.get("id") == did:
            d["last"] = now
            d["label"] = lab or d.get("label") or "device"
            found = True
            break
    if not found:
        devices.append({"id": did, "label": lab, "first": now, "last": now})
    devices = devices[-20:]
    users_col.update_one({"_id": user["_id"]}, {"$set": {"devices": devices}})
    return {"ok": True, "count": len(devices)}

@app.get("/mod/devices")
async def mod_devices(username: str = Query(...), user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    u = users_col.find_one({"username": username.strip().lstrip("@").lower()})
    if not u:
        raise HTTPException(404, "User not found")
    return {
        "username": u["username"],
