...
client = db = users_col = pixels_col = clans_col = chats_col = zones_col = None
online_count = 0
...
async def lifespan(app: FastAPI):
    global client, db, users_col, pixels_col, clans_col, chats_col, zones_col
    if MONGO_URI:
...
async def place_pixel(data: PlacePixel, user=Depends(get_current_user)):
    ...
    await manager.broadcast({"type": "pixel", "x": x, "y": y, "color": color, "username": user["username"]})
...
async def earn_start(user=Depends(get_current_user)):
    ...
    mode = "math" if random.random() < 0.5 else "animal"
    ...
    if streak > 20 and random.random() < 0.63:
        ...
        b = random.randint(1, min(99 - a, 30))
    correct = str(a + b)
    wrongs = set()
    while len(wrongs) < 8:
        w = a + b + random.randint(-15, 15)
        if w != a + b and 0 <= w <= 120:
            wrongs.add(str(w))
    ...
    if len(earn_sessions) > 20000:
        earn_sessions.clear()
    return {"session_id": sid, "mode": "math", "question": f"{a} + {b}", "options": options, "cooldown": 15}
...
async def send_chat(message: str = Body(..., embed=True), channel: str = Body("en"), user=Depends(get_current_user)):
    ...
    if chats_col is not None:
        chats_col.delete_many({"created_at": {"$lt": datetime.utcnow() - timedelta(hours=1)}})
    doc = {
...
async def chat_history(channel: str = "en", limit: int = 50):
...
async def health():
    ...
    try:
        ...
        listing = [str(e)]
    return {"status": "ok", "mongo": users_col is not None, "index_html": _find_file("index.html") is not None,
            "world_map": _find_file("world_map.png") is not None, "files": listing, "online": online_count}
