...
async def register(data: RegisterBody):
    ...
    if len(uname) < 3 or len(uname) > 20:
        raise HTTPException(400, "Username 3-20 latin (a-z, 0-9, _)")
...
# ---------- EARN: animal → color → math by streak ----------
...
EARN_COLORS = ["#000000","#e4e4e4","#c4c4c4","#888888","#4e4e4e","#e50000","#ff2323","#02be01","#0000ea","#ffca00","#820080","#50a3d7","#ff54b2","#fc9a29","#00d3dd","#940000","#6e4bed"]
...
async def earn_start(user=Depends(get_current_user)):
    ...
    # rotate by streak: animal, color, math
    mode_i = streak % 3
    if mode_i == 0:
...
async def publish_template(data: PublicTemplate, user=Depends(get_current_user)):
    ...
    owned = public_tpl_col.count_documents({"author": user["username"]})
    if owned >= 2:
        raise HTTPException(400, "Max 2 published templates")
...
async def send_chat(message: str = Body(..., embed=True), channel: str = Body("en"), user=Depends(get_current_user)):
    ...
    # spam: same message 3 times → mute 2 min; after 4 min can repeat that text
    spam_until = user.get("spam_mute_until")
    if spam_until and isinstance(spam_until, datetime) and spam_until > datetime.utcnow():
        left = int((spam_until - datetime.utcnow()).total_seconds() // 60) + 1
        raise HTTPException(403, f"Spam mute {left}m")
    last_msgs = list(user.get("last_chat_msgs") or [])
    msg_norm = (message or "").strip().lower()
    last_msgs = (last_msgs + [msg_norm])[-5:]
    if last_msgs.count(msg_norm) >= 3:
        until = datetime.utcnow() + timedelta(minutes=2)
        users_col.update_one({"_id": user["_id"]}, {"$set": {"spam_mute_until": until, "last_chat_msgs": last_msgs, "spam_blocked_text": msg_norm, "spam_blocked_at": datetime.utcnow()}})
        raise HTTPException(403, "Spam: muted 2 minutes")
    # after 4 min from block, allow same text again
    blocked_at = user.get("spam_blocked_at")
    blocked_text = (user.get("spam_blocked_text") or "")
    if blocked_text and msg_norm == blocked_text and blocked_at:
        try:
            if isinstance(blocked_at, datetime) and (datetime.utcnow() - blocked_at).total_seconds() < 240:
                # still within 4 min window but not currently muted — block that exact text until 4m
                if spam_until and spam_until > datetime.utcnow():
                    pass
                elif (datetime.utcnow() - blocked_at).total_seconds() < 240 and not (spam_until and spam_until > datetime.utcnow()):
                    # allow only if 4 min passed - if under 4 min after unmute, still block same text
                    if (datetime.utcnow() - blocked_at).total_seconds() < 240:
                        raise HTTPException(403, "Wait before repeating that message")
        except HTTPException:
            raise
        except Exception:
            pass
    users_col.update_one({"_id": user["_id"]}, {"$set": {"last_chat_msgs": last_msgs}})
    if channel not in ("global", "en", "ru", "ar", "tr", "clan") and not str(channel).startswith("dm:"):
        channel = "en"
    doc = {
        "username": user["username"],
        "display_name": user.get("display_name") or user["username"],
    ...
    if chats_col is not None:
        chats_col.insert_one(doc)
        keep = 60 if channel == "clan" else (70 if str(channel).startswith("dm:") else 30)
...
async def chat_history(channel: str = "en", limit: int = 100):
    if chats_col is None:
        return []
    lim = 60 if channel == "clan" else (70 if str(channel).startswith("dm:") else 30)
