...
async def get_all_pixels():
    """Full map dump disabled when too large (kills free Render). Use /canvas/chunk."""
    est = 0
    if pixels_col is not None:
        try:
            est = pixels_col.estimated_document_count()
    ...
    if est > 25000:
        return {
            "xs": [], "ys": [], "cs": [], "count": 0,
            "disabled": True,
            "reason": "map_too_large_use_chunks",
            "pixels_in_db": est,
        }
    xs, ys, cs = [], [], []
    if pixels_col is not None:
        try:
            for doc in pixels_col.find({}, {"_id": 0, "x": 1, "y": 1, "color": 1}).batch_size(4000):
                try:
                    xs.append(int(doc["x"]))
                    ys.append(int(doc["y"]))
                    cs.append(doc["color"])
