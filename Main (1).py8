...
async def get_chunk(x1: int = 0, y1: int = 0, x2: int = 512, y2: int = 512):
    ...
    # Prefer direct Mongo for chunks (reliable even if RAM cache lagging)
    if pixels_col is not None:
        try:
...
async def get_all_pixels():
    """All painted pixels. Streams from Mongo (no double RAM copy) to avoid 512MB OOM."""
    xs, ys, cs = [], [], []
    # Prefer Mongo cursor — do NOT force full RAM cache first
    if pixels_col is not None:
        try:
    ...
    # fallback: in-memory cache if available
    if _pixel_cache:
        for k, col in _pixel_cache.items():
            try:
                a, b = k.split(",")
