...
# Shared palette indices (0-based). Unknown colors still use hex in WS.
# Keep in sync with client ALL[] order (lowercase).
COLOR_TABLE = [
    "#000000","#ffffff","#e4e4e4","#c4c4c4","#888888","#4e4e4e",
    "#9a0000","#e50000","#ff6565","#f4b3ae","#ffa7d1","#ff54b2",
    "#fea460","#e59500","#a06a42","#604028",
    ...
    "#3989c7","#ffca00","#ff2323",
]
COLOR_TO_IDX = {c: i for i, c in enumerate(COLOR_TABLE)}
IDX_TO_COLOR = {i: c for i, c in enumerate(COLOR_TABLE)}

def color_to_wire(c: str):
    """Prefer small int index over hex string for WS."""
    c = (c or "").lower()
    if c in COLOR_TO_IDX:
        return COLOR_TO_IDX[c]
    return c

def wire_to_color(v) -> str:
    if isinstance(v, int):
        return IDX_TO_COLOR.get(v, "#000000")
    if isinstance(v, float) and v == int(v):
        return IDX_TO_COLOR.get(int(v), "#000000")
    s = str(v or "").strip().lower()
    if s.isdigit():
        return IDX_TO_COLOR.get(int(s), "#000000")
    return s if s.startswith("#") else ("#" + s if len(s) == 6 else s)
MAX_PIXELS_AUTH = 200
...
# Anti-lag: per-user + GLOBAL place rate limits
_place_rate: Dict[str, list] = {}  # username -> [timestamps]
_place_rate_global: list = []
PLACE_RATE_WINDOW = 1.0  # seconds
PLACE_RATE_MAX = 60  # max pixels per user per second
PLACE_RATE_GLOBAL_MAX = 250  # max pixels server-wide per second

def _check_place_rate(username: str, n: int = 1) -> int:
    """Return how many of n pixels allowed (user + global). 0 = none."""
    global _place_rate_global
    now = datetime.utcnow().timestamp()
    arr = [t for t in (_place_rate.get(username) or []) if now - t < PLACE_RATE_WINDOW]
    g = [t for t in _place_rate_global if now - t < PLACE_RATE_WINDOW]
    room = min(max(0, PLACE_RATE_MAX - len(arr)), max(0, PLACE_RATE_GLOBAL_MAX - len(g)))
    if room <= 0:
        _place_rate[username] = arr
        _place_rate_global = g
        return 0
    take = min(n, room)
    arr.extend([now] * take)
    g.extend([now] * take)
    _place_rate[username] = arr[-PLACE_RATE_MAX:]
    _place_rate_global = g[-PLACE_RATE_GLOBAL_MAX:]
    return take

def _compact_px(placed: list) -> dict:
    """Compact WS: color as palette index when possible (smaller than #rrggbb)."""
    return {"type": "px", "p": [[p["x"], p["y"], color_to_wire(p["color"])] for p in placed]}
...
# anti-spam: username -> {msg, count, level, last_ts, mute_until_spam}
_chat_spam: Dict[str, dict] = {}
...
class PlacePixel(BaseModel):
    x: int
    y: int
    color: str  # #hex or decimal index as string e.g. "3"
    ...
    if not MONGO_URI:
        ...
        try:
            ...
            try:
                ...
                users_col.create_index([("pixels_placed", DESCENDING)])
                users_col.create_index([("day_key", ASCENDING), ("pixels_placed_day", DESCENDING)])
                clans_col.create_index("tag", unique=True)
                chats_col.create_index("created_at")
                zones_col.create_index("expires_at")
                users_col.create_index("device_fp")
...
class ConnectionManager:
    def __init__(self):
        ...
        self.viewports: Dict[int, dict] = {}  # id(ws) -> {x1,y1,x2,y2}
    async def connect(self, ws: WebSocket):
        ...
        self.viewports[id(ws)] = {"x1": 0, "y1": 0, "x2": CANVAS_SIZE, "y2": CANVAS_SIZE}
    def disconnect(self, ws: WebSocket):
        global online_count
        if ws in self.active:
            self.active.remove(ws)
        online_users.pop(id(ws), None)
        self.viewports.pop(id(ws), None)
        online_count = len(self.active)
    def identify(self, ws: WebSocket, username: str, display_name: str):
        ...
        if users_col is not None:
            try:
                ...
                pass
    def set_viewport(self, ws: WebSocket, x1, y1, x2, y2, pad=128):
        self.viewports[id(ws)] = {
            "x1": max(0, int(x1) - pad), "y1": max(0, int(y1) - pad),
            "x2": min(CANVAS_SIZE, int(x2) + pad), "y2": min(CANVAS_SIZE, int(y2) + pad),
        }
    def _px_in_view(self, vp: dict, x: int, y: int) -> bool:
        if not vp:
            return True
        return vp["x1"] <= x < vp["x2"] and vp["y1"] <= y < vp["y2"]
    async def broadcast(self, message: dict):
        """Pixel packs (type px) only to clients whose viewport intersects the pixels."""
        dead = []
        is_px = message.get("type") == "px" and message.get("p")
        data_full = json.dumps(message, default=str)
        for ws in self.active:
            try:
                if is_px:
                    vp = self.viewports.get(id(ws))
                    # filter pixels to this viewport
                    filtered = [p for p in message["p"] if self._px_in_view(vp, p[0], p[1])]
                    if not filtered:
                        continue
                    if len(filtered) == len(message["p"]):
                        await ws.send_text(data_full)
                    else:
                        await ws.send_text(json.dumps({"type": "px", "p": filtered}, default=str))
                else:
                    await ws.send_text(data_full)
            except Exception:
                dead.append(ws)
...
_user_cache: Dict[str, tuple] = {}  # username -> (user_dict, ts)
USER_CACHE_TTL = 3.0
...
# rank cache: username -> {daily, alltime, ts}
_rank_cache: Dict[str, dict] = {}
RANK_CACHE_TTL = 45.0  # seconds — avoid 2 slow count_documents on every /me

def get_ranks(user):
    """Return daily and alltime rank. Cached ~45s to keep /me fast under load."""
    if not user or users_col is None:
        return {"daily": None, "alltime": None}
    uname = user.get("username") or ""
    now = datetime.utcnow().timestamp()
    cached = _rank_cache.get(uname)
    if cached and (now - cached.get("ts", 0)) < RANK_CACHE_TTL:
        return {"daily": cached.get("daily"), "alltime": cached.get("alltime")}
    uid = user["_id"]
    day_placed = user.get("pixels_placed_day", 0) if user.get("day_key") == day_key() else 0
    all_placed = user.get("pixels_placed", 0)
    daily_rank = None
    alltime_rank = None
    try:
        if day_placed > 0:
            # simpler + indexed-friendly: count higher scores only
        ...
        print("get_ranks err", e)
    _rank_cache[uname] = {"daily": daily_rank, "alltime": alltime_rank, "ts": now}
    # prune
    if len(_rank_cache) > 500:
        cutoff = now - RANK_CACHE_TTL
        for k in list(_rank_cache.keys())[:200]:
            if _rank_cache[k].get("ts", 0) < cutoff:
...
def norm_color(c) -> str:
    """Accept #hex or palette index (int/str digit). Always return #rrggbb for DB."""
    if isinstance(c, int) or (isinstance(c, float) and c == int(c)):
        return IDX_TO_COLOR.get(int(c), "#000000")
    s = str(c or "").strip().lower()
    if s.isdigit():
        return IDX_TO_COLOR.get(int(s), "#000000")
    if len(s) == 4 and s.startswith("#"):
        s = "#" + s[1]*2 + s[2]*2 + s[3]*2
    if s in COLOR_TO_IDX:
        return s
    return s
...
def _client_ip(request: Request) -> str:
    """Best-effort client IP behind Render/nginx proxies."""
    try:
        xf = request.headers.get("x-forwarded-for") or request.headers.get("X-Forwarded-For") or ""
        if xf:
...
def _ip_banned(ip: str):
    if not ip or ip == "unknown" or db is None:
        return None
    try:
        col = db["banned_ips"]
        b = col.find_one({"ip": ip})
        if not b:
            return None
        until = b.get("until")
...
# ---------- AUTH ----------
@app.post("/register")
async def register(data: RegisterBody, request: Request):
    ...
    # IP ban check (2 weeks after multi-account abuse)
    ip_ban = _ip_banned(ip)
    if ip_ban:
        raise HTTPException(403, "This IP is banned for multi-accounting")
    # Anti multi-account: 1 account per device fingerprint per week
    fp = (data.device_fp or "").strip()[:128]
    if fp:
        try:
            banned = _device_banned(fp)
        except Exception:
            banned = None
        if banned:
            raise HTTPException(403, "This device is banned")
        week_ago = datetime.utcnow() - timedelta(days=7)
        recent = users_col.find_one({
            "device_fp": fp,
            "created_at": {"$gte": week_ago},
            "is_mod": {"$ne": True},
        })
        if recent:
            raise HTTPException(403, "Only 1 account per device per week")
    # Same IP: if already 2 non-mod accounts, 3rd triggers wipe + IP ban 14 days
    if ip and ip != "unknown":
        same_ip = list(users_col.find({"reg_ip": ip, "is_mod": {"$ne": True}}, {"username": 1, "_id": 1}))
        if len(same_ip) >= 2:
            # this would be the 3rd → ban IP 14 days, delete all accounts from this IP
            until = datetime.utcnow() + timedelta(days=14)
            if db is not None:
                db["banned_ips"].update_one(
                    {"ip": ip},
                    {"$set": {"ip": ip, "until": until, "reason": "3+ accounts same IP", "created": datetime.utcnow()}},
                    upsert=True,
                )
            deleted = 0
            for u in same_ip:
                ...
                try:
                    if pixels_col is not None:
                        pixels_col.delete_many({"username": un})
                    if chats_col is not None:
                        chats_col.delete_many({"username": un})
                    if public_tpl_col is not None:
                        public_tpl_col.delete_many({"author": un})
                    users_col.delete_one({"_id": u["_id"]})
                    deleted += 1
                except Exception as e:
                    print("ip multi wipe", un, e)
            raise HTTPException(403, f"Multi-account from same IP blocked. IP banned 14 days. Deleted {deleted} accounts.")
...
async def login(form: OAuth2PasswordRequestForm = Depends()):
    ...
    if user.get("banned"):
        ...
        if until and isinstance(until, datetime) and until < datetime.utcnow():
            users_col.update_one({"_id": user["_id"]}, {"$set": {"banned": False}, "$unset": {"banned_until": "", "ban_reason": "", "banned_by": ""}})
        else:
...
async def profile_update(data: UpdateProfile, user=Depends(get_current_user)):
    ...
    if getattr(data, "name_color", None) is not None:
        upd["name_color"] = max(0, min(9, int(data.name_color)))
    if not upd:
...
async def place_pixel(data: PlacePixel, user=Depends(get_current_user)):
    ...
    # same color = free (RAM only, no Mongo find_one)
    ensure_pixel_cache()
    if _pixel_cache is not None and norm_color(_pixel_cache.get(f"{x},{y}") or "") == color:
    ...
    # user counters (one update)
    _user_cache.pop(uname, None)
    if not user.get("god_mode"):
        users_col.update_one({"_id": user["_id"]}, {
            "$inc": {"pixels_left": -1, "pixels_placed": 1, "pixels_placed_day": 1},
            "$set": {"day_key": day_key(), "last_seen": now,
                     "last_pixel": {"x": x, "y": y, "color": color, "t": now.isoformat()}}
        })
    else:
        users_col.update_one({"_id": user["_id"]}, {
            "$inc": {"pixels_placed": 1, "pixels_placed_day": 1},
            "$set": {"day_key": day_key(), "last_seen": now,
                     "last_pixel": {"x": x, "y": y, "color": color, "t": now.isoformat()}}
        })
    _push_recent_pixel(x, y, color, uname)
    await manager.broadcast(_compact_px([{"x": x, "y": y, "color": color}]))
    # Mongo write in background — client already got answer path
    if pixels_col is not None:
        def _w():
            try:
                pixels_col.update_one({"x": x, "y": y}, {"$set": {
                    "color": color, "user_id": uid, "username": uname,
                    "display_name": dname, "placed_at": now
                }}, upsert=True)
            except Exception as e:
                print("place bg write", e)
        try:
            asyncio.create_task(asyncio.to_thread(_w))
        except Exception:
            _w()
...
async def place_batch(data: PlaceBatch, user=Depends(get_current_user)):
    """Fast batch place: validate in memory, one bulk_write, reply quickly."""
    if not user:
        raise HTTPException(401, "NOT_AUTHORIZED")
    if user.get("banned"):
        raise HTTPException(403, "Banned")
    ...
    if room <= 0:
        raise HTTPException(429, "Rate limit")
    pts = pts[:room]
    left_budget = 10**9 if user.get("god_mode") else int(user.get("pixels_left") or 0)
    placed = []
    ops = []
    now = datetime.utcnow()
    uname = user["username"]
    dname = user.get("display_name") or uname
    uid = str(user["_id"])
    ensure_pixel_cache()  # once for whole batch
    for p in pts:
        if len(placed) >= left_budget:
            break
        ...
        if check_zone(x, y):
            continue
        # skip same color via RAM cache only (no per-pixel Mongo find)
        if _pixel_cache is not None and norm_color(_pixel_cache.get(f"{x},{y}") or "") == color:
            continue
        cache_set_pixel(x, y, color)
        ...
        ops.append(UpdateOne(
            {"x": x, "y": y},
            {"$set": {"color": color, "user_id": uid, "username": uname, "display_name": dname, "placed_at": now}},
            upsert=True
        ))
    if not placed:
        return {"ok": True, "placed": 0, "skipped": 0, "pixels_left": user.get("pixels_left", 0)}
    inc = 0 if user.get("god_mode") else len(placed)
    _user_cache.pop(user.get("username") or "", None)
    users_col.update_one({"_id": user["_id"]}, {
        "$inc": {"pixels_left": -inc, "pixels_placed": len(placed), "pixels_placed_day": len(placed)},
        "$set": {
            "day_key": day_key(),
            "last_seen": now,
            "last_pixel": {"x": placed[-1]["x"], "y": placed[-1]["y"], "color": placed[-1]["color"], "t": now.isoformat()},
        }
    })
    _push_recent_pixel(placed[-1]["x"], placed[-1]["y"], placed[-1]["color"], uname)
    await manager.broadcast(_compact_px(placed))
    # Mongo bulk in background so client gets answer fast
    if ops and pixels_col is not None:
        def _bw():
            try:
                pixels_col.bulk_write(ops, ordered=False)
            except Exception as e:
                print("bulk_write err", e)
        try:
            asyncio.create_task(asyncio.to_thread(_bw))
        except Exception:
            _bw()
    left = int(user.get("pixels_left") or 0) - inc
    return {"ok": True, "placed": len(placed), "skipped": 0, "pixels_left": left}

@app.get("/lookup")
async def lookup(x: int = Query(...), y: int = Query(...), user=Depends(get_current_user)):
    """Who placed the pixel — available to all logged-in players (basic info)."""
    if not user:
        raise HTTPException(401, "Login required")
    if not (0 <= x < CANVAS_SIZE and 0 <= y < CANVAS_SIZE):
        return {"empty": True}
    doc = None
    if pixels_col is not None:
        doc = pixels_col.find_one({"x": int(x), "y": int(y)})
    if not doc:
        ensure_pixel_cache()
        col = (_pixel_cache or {}).get(f"{x},{y}")
        if not col:
            return {"empty": True, "x": x, "y": y}
        return {"empty": False, "x": x, "y": y, "color": col, "username": None, "display_name": "Unknown"}
    ...
    profile = {
        ...
        "color": doc.get("color"),
        "username": uname,
        "display_name": doc.get("display_name") or uname,
        "placed_at": str(doc.get("placed_at") or ""),
    ...
    if uname and users_col is not None:
        ...
        if u:
            pub = user_public(u)
            profile.update({
                "display_name": pub.get("display_name") or uname,
                "avatar_color": pub.get("avatar_color", 0),
                "name_color": pub.get("name_color", 0),
                "medals": pub.get("medals") or {},
                "pixels_placed": pub.get("pixels_placed", 0),
                "is_mod": pub.get("is_mod", False),
                "clan_tag": pub.get("clan_tag"),
                "banned": pub.get("banned", False),
            })
            if user and user.get("is_mod"):
                profile["device_fp"] = u.get("device_fp")
                profile["last_seen"] = str(u.get("last_seen") or "")
...
@app.get("/canvas/region")
async def canvas_region(x1: int = 0, y1: int = 0, x2: int = 512, y2: int = 512):
    """Load only a rectangle of the map (for viewport). Max 1024x1024 area."""
    x1 = max(0, min(CANVAS_SIZE, int(x1))); y1 = max(0, min(CANVAS_SIZE, int(y1)))
    x2 = max(0, min(CANVAS_SIZE, int(x2))); y2 = max(0, min(CANVAS_SIZE, int(y2)))
    if x2 < x1: x1, x2 = x2, x1
    if y2 < y1: y1, y2 = y2, y1
    # hard cap area
    if (x2 - x1) * (y2 - y1) > 1024 * 1024:
        x2 = x1 + min(1024, x2 - x1)
        y2 = y1 + min(1024, (1024 * 1024) // max(1, x2 - x1))
    await asyncio.to_thread(ensure_pixel_cache)
    xs, ys, cs = [], [], []
    if _pixel_cache:
        for key, col in _pixel_cache.items():
            try:
                a, b = key.split(",")
                xi, yi = int(a), int(b)
                if x1 <= xi < x2 and y1 <= yi < y2:
                    xs.append(xi); ys.append(yi); cs.append(col)
            except Exception:
                pass
    return {"xs": xs, "ys": ys, "cs": cs, "count": len(xs), "x1": x1, "y1": y1, "x2": x2, "y2": y2}
...
@app.get("/stats")
async def stats():
    total_pixels = (pixels_col.estimated_document_count() if pixels_col is not None else 0)
...
async def mod_ban(data: ModAction, user=Depends(get_current_user)):
    ...
    upd = {"banned": True, "ban_reason": data.reason or "", "banned_by": user["username"]}
    if minutes > 0:
        upd["banned_until"] = datetime.utcnow() + timedelta(minutes=min(minutes, 365*24*60))
    else:
        upd["banned_until"] = None  # permanent
    users_col.update_one({"username": target}, {"$set": upd})
    return {"ok": True, "minutes": minutes or "permanent"}
...
async def save_templates(data: TemplateSave, user=Depends(get_current_user)):
    if not user:
        raise HTTPException(401)
    # max 7 personal templates (admin unlimited)
    max_tpl = 999 if user.get("is_mod") else 7
    tpls = data.templates[:max_tpl]
    clean = []
    for t in tpls:
        ...
        clean.append({"x": int(t.get("x") or 0), "y": int(t.get("y") or 0),
                      ...
                      "opacity": float(t.get("opacity") if t.get("opacity") is not None else 0.45)})
...
async def publish_template(data: PublicTemplate, user=Depends(get_current_user)):
    ...
    # max 2 public templates per user (admin unlimited)
    existing = public_tpl_col.count_documents({"author": user["username"]})
    if existing >= 2 and not user.get("is_mod"):
        raise HTTPException(400, "Max 2 public templates — delete old first")
...
async def _process_mod_command(raw: str, user: dict) -> Optional[dict]:
    ...
    global _global_event, _window_sms, _user_event_bonus
    ...
    if low in ("/abuse_day", "/abuse_day_time"):
        _global_event = {
            "mode": None, "ends_at": None, "overlay": False, "set_by": user["username"],
            "opacity": 0.8, "color": None,
            "earn_mul": 1.0, "earn_add": 0.0, "afk_mul": 1.0, "afk_add": 0.0, "stock_add": 0,
            "select": None, "pixel_cost": 1.0,
        }
        _user_event_bonus.clear()
        await manager.broadcast({"type": "event", "mode": None, "ended": True, "by": user["username"], "clear_select": True})
        return {"ok": True, "silent": True, "msg": "Event ended — normal (select cleared)"}
...
async def send_chat(message: str = Body(..., embed=True), channel: str = Body("en"), user=Depends(get_current_user)):
    ...
    if channel not in ("en", "ru", "clan") and not str(channel).startswith("dm:"):
        channel = "en"
    ...
    if not user.get("is_mod"):
        ...
        st = _chat_spam.get(key) or {"msg": "", "count": 0, "level": 0, "last_ts": 0}
        # reset history after 1 hour
        if now_ts - float(st.get("last_ts") or 0) > 3600:
            st = {"msg": "", "count": 0, "level": 0, "last_ts": 0}
        if st.get("msg") == msg_n and msg_n:
            st["count"] = int(st.get("count") or 0) + 1
            st["last_ts"] = now_ts
            # 2nd same -> 2 min; then 10, 20, 30...
            # 3rd identical message triggers mute (was 2)
            if st["count"] >= 3:
                if st["level"] == 0:
                    mins = 2
                    st["level"] = 1
                else:
                    mins = 10 + (st["level"] - 1) * 10
                    st["level"] = int(st["level"]) + 1
                until = datetime.utcnow() + timedelta(minutes=mins)
                users_col.update_one({"_id": user["_id"]}, {"$set": {"muted_until": until, "muted_by": "antispam"}})
                _chat_spam[key] = st
                # bot message
                bot = {
                    "username": "settings", "display_name": "SETTINGS",
                    "message": f"@{user['username']} muted {mins}m for spam",
                    "channel": channel, "is_mod": True, "avatar_color": 0,
                    "created_at": datetime.utcnow(),
                }
                if chats_col is not None:
                    chats_col.insert_one(bot)
                await manager.broadcast({"type": "chat", "username": bot["username"], "display_name": bot["display_name"],
                    "message": bot["message"], "channel": channel, "is_mod": True,
                    "avatar_color": 0, "created_at": str(bot["created_at"])})
                raise HTTPException(403, f"MUTED_BY:@antispam|MIN:{mins}")
        else:
            st = {"msg": msg_n, "count": 1, "level": st.get("level") or 0, "last_ts": now_ts}
        _chat_spam[key] = st
    # Pin message if starts with apostrophe '
    is_pin = message.startswith("'")
    pin_until = None
    if is_pin:
        last_pin = user.get("last_pin_at")
        if last_pin and isinstance(last_pin, datetime) and (datetime.utcnow() - last_pin).total_seconds() < 20 * 60:
            raise HTTPException(400, "Pin cooldown 20 min")
        if not user.get("is_mod"):
            users_col.update_one({"_id": user["_id"]}, {"$set": {"last_pin_at": datetime.utcnow()}})
        pin_until = datetime.utcnow() + timedelta(hours=2)
        message = message[1:].lstrip() or message  # strip leading '
    doc = {
        "username": user["username"],
        "display_name": user.get("display_name") or user["username"],
        "message": message[:200],
    ...
    if chats_col is not None:
        chats_col.insert_one(doc)
        # purge expired pins + keep last 25 non-pinned
        now = datetime.utcnow()
        chats_col.delete_many({"pinned": True, "pin_until": {"$lt": now}})
        keep = 25
        extra = list(chats_col.find({"channel": channel, "pinned": {"$ne": True}}).sort("created_at", DESCENDING).skip(keep))
        if extra:
            chats_col.delete_many({"_id": {"$in": [e["_id"] for e in extra]}})
    await manager.broadcast({"type": "chat", "username": doc["username"], "display_name": doc["display_name"],
        "message": doc["message"], "channel": channel, "is_mod": doc["is_mod"],
        "avatar_color": doc["avatar_color"], "name_color": doc.get("name_color", 0),
        "created_at": str(doc["created_at"]), "pinned": bool(is_pin)})
    return {"ok": True, "pinned": bool(is_pin)}

@app.get("/chat/history")
async def chat_history(channel: str = "en", limit: int = 30):
    if chats_col is None:
        return []
    if channel not in ("en", "ru", "clan") and not channel.startswith("dm:"):
        channel = "en"
    # purge expired pins
    try:
        chats_col.delete_many({"pinned": True, "pin_until": {"$lt": datetime.utcnow()}})
    except Exception:
        pass
    lim = 40
    # pins first, then recent
    pins = list(chats_col.find({"channel": channel, "pinned": True, "pin_until": {"$gt": datetime.utcnow()}}).sort("created_at", DESCENDING).limit(10))
    cursor = chats_col.find({"channel": channel, "pinned": {"$ne": True}}).sort("created_at", DESCENDING).limit(min(limit, 30))
    msgs = list(cursor)[::-1]
    out = []
    for c in pins[::-1] + msgs:
        out.append({
            "username": c["username"], "display_name": c.get("display_name"), "message": c["message"],
            "is_mod": c.get("is_mod", False), "avatar_color": c.get("avatar_color", 0),
            "name_color": c.get("name_color", 0),
...
async def clan_boost(user=Depends(get_current_user)):
    """Overlord / KING Team-Boost: +50 to all online clan members. CD 5h. Requires 20 members."""
    if not user or not user.get("clan_id"):
        raise HTTPException(400, "No clan")
    clan = clans_col.find_one({"_id": ObjectId(user["clan_id"])})
    if not clan:
    ...
    if len(members) < 20:
        raise HTTPException(400, "Need 20 members for KING Team-Boost")
    # grant king medal
    users_col.update_one({"_id": user["_id"]}, {"$set": {"king_medal": True}})
        ...
        while True:
            ...
            try:
                ...
                if msg.get("type") == "viewport":
                    try:
                        manager.set_viewport(ws, msg.get("x1", 0), msg.get("y1", 0), msg.get("x2", CANVAS_SIZE), msg.get("y2", CANVAS_SIZE))
                    except Exception:
                        pass
            except Exception:
                pass
...
@app.get("/")
async def root():
    for name in ("index.html", "index.htmll", "index.html$", "index.html-", "index.HTML"):
        p = _find_file(name)
        if p:
            return FileResponse(p, media_type="text/html")
    # last resort: scan for any index*.html
    for root in (os.path.dirname(os.path.abspath(__file__)), os.getcwd(), "/opt/render/project/src"):
        try:
            for fn in os.listdir(root):
                if fn.lower().startswith("index") and fn.lower().endswith((".html", ".htm")):
                    return FileResponse(os.path.join(root, fn), media_type="text/html")
        except Exception:
            pass
    return HTMLResponse(
        "<h2>index.html not found</h2><p>Upload <b>index.html</b> next to main.py on Render.</p>",
        status_code=404,
    )
...
@app.post("/mod/ban_hardware")
async def mod_ban_hardware(data: dict = Body(...), user=Depends(get_current_user)):
    """Ban device fingerprint — blocks register on that device."""
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    target = str(data.get("target") or "").strip().lstrip("@").lower()
    minutes = int(data.get("minutes") or 10080)
    reason = str(data.get("reason") or "hardware ban")[:200]
    if not target:
        raise HTTPException(400, "Target required")
    ...
    fp = t.get("device_fp")
    if not fp:
        raise HTTPException(400, "No device fingerprint on this account")
    until = datetime.utcnow() + timedelta(minutes=max(1, min(minutes, 365 * 24 * 60)))
    if db is not None:
        ban_col = db["banned_devices"]
        ban_col.update_one(
            {"fp": fp},
            {"$set": {
                "fp": fp, "until": until, "by": user["username"],
                "reason": reason, "target": target, "created": datetime.utcnow(),
            }},
            upsert=True,
        )
    users_col.update_one({"_id": t["_id"]}, {"$set": {
        "banned": True, "ban_reason": reason, "banned_by": user["username"],
        "banned_until": until, "hardware_banned": True,
    }})
    return {"ok": True, "fp": (fp[:12] + "…") if fp else "", "until": str(until)}

def _device_banned(fp: str):
    if not fp or db is None:
        return None
    try:
        ban_col = db["banned_devices"]
        b = ban_col.find_one({"fp": fp})
        if not b:
            return None
        until = b.get("until")
        if until and isinstance(until, datetime) and until < datetime.utcnow():
            ban_col.delete_one({"fp": fp})
            return None
        return b
    except Exception:
        return None
...
# ---------- DM / DELETE / KING ----------
@app.post("/chat/dm")
async def send_dm(data: dict = Body(...), user=Depends(get_current_user)):
    """DM only if target is currently online."""
    if not user:
        raise HTTPException(401)
    target = str(data.get("target") or "").strip().lstrip("@").lower()
    ...
    for info in online_users.values():
        if (info.get("username") or "").lower() == target:
            ...
            break
    if not online:
        raise HTTPException(400, "User is offline — DM only when online")
    muted = user.get("muted_until")
    if muted and muted > datetime.utcnow():
        raise HTTPException(403, "Muted")
    # channel key sorted so both see same history
    a, b = sorted([user["username"], target])
    ch = f"dm:{a}:{b}"
    doc = {
        "username": user["username"],
        "display_name": user.get("display_name") or user["username"],
    ...
    if chats_col is not None:
        chats_col.insert_one(doc)
        extra = list(chats_col.find({"channel": ch}).sort("created_at", DESCENDING).skip(30))
        if extra:
            chats_col.delete_many({"_id": {"$in": [e["_id"] for e in extra]}})
    await manager.broadcast({
        "type": "chat", "username": doc["username"], "display_name": doc["display_name"],
        "message": doc["message"], "channel": ch, "is_mod": doc["is_mod"],
        "avatar_color": doc["avatar_color"], "name_color": doc.get("name_color", 0),
        "created_at": str(doc["created_at"]), "dm_to": target, "dm_from": user["username"],
    })
    return {"ok": True, "channel": ch}

@app.post("/chat/delete")
async def chat_delete(data: dict = Body(...), user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    mid = str(data.get("id") or "")
    if not mid or chats_col is None:
        raise HTTPException(400)
    try:
        chats_col.delete_one({"_id": ObjectId(mid)})
    except Exception:
        raise HTTPException(400, "Bad id")
    await manager.broadcast({"type": "chat_del", "id": mid})
    return {"ok": True}

@app.post("/mod/delete_report")
async def mod_delete_report(data: dict = Body(...), user=Depends(get_current_user)):
    _ensure_extra_cols()
    if not user or not user.get("is_mod"):
        raise HTTPException(403)
    try:
        reports_col.delete_one({"_id": ObjectId(str(data.get("id")))})
    except Exception:
        raise HTTPException(400)
    return {"ok": True}

@app.post("/mod/delete_question")
async def mod_delete_question(data: dict = Body(...), user=Depends(get_current_user)):
    _ensure_extra_cols()
    if not user or not user.get("is_mod"):
        raise HTTPException(403)
    try:
        questions_col.delete_one({"_id": ObjectId(str(data.get("id")))})
    except Exception:
        raise HTTPException(400)
    return {"ok": True}

@app.post("/mod/delete_video")
async def mod_delete_video(data: dict = Body(...), user=Depends(get_current_user)):
    _ensure_extra_cols()
    if not user or not user.get("is_mod"):
        raise HTTPException(403)
    try:
        videos_col.delete_one({"_id": ObjectId(str(data.get("id")))})
    except Exception:
        raise HTTPException(400)
    return {"ok": True}

@app.post("/news/delete_comment")
async def news_delete_comment(data: dict = Body(...), user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403)
    if news_col is None:
        raise HTTPException(503)
    try:
        oid = ObjectId(str(data.get("nid")))
    except Exception:
        raise HTTPException(400)
    ...
    if not n:
        raise HTTPException(404)
    comments = list(n.get("comments") or [])
    if 0 <= idx < len(comments):
        comments.pop(idx)
        news_col.update_one({"_id": oid}, {"$set": {"comments": comments}})
    return {"ok": True}

@app.post("/clan/refresh_king")
async def refresh_king(user=Depends(get_current_user)):
    """Set king_medal if user is leader of clan with 20+ members."""
    if not user or not user.get("clan_id"):
        return {"king": False}
    clan = clans_col.find_one({"_id": ObjectId(user["clan_id"])})
    if not clan:
        return {"king": False}
    is_king = (clan.get("leader") == user["username"] and len(clan.get("members") or []) >= 20)
    users_col.update_one({"_id": user["_id"]}, {"$set": {"king_medal": is_king}})
    return {"king": is_king, "members": len(clan.get("members") or [])}



@app.post("/mod/cleanup_inactive")
async def cleanup_inactive(user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    return await _do_cleanup_inactive()

async def _do_cleanup_inactive():
    if users_col is None:
        return {"ok": True, "deleted": 0}
    ...
    q = {
        ...
        "$or": [
            {"last_seen": {"$lt": cutoff}},
            {"last_seen": {"$exists": False}, "created_at": {"$lt": cutoff}},
        ],
    }
    to_del = list(users_col.find(q, {"username": 1, "_id": 1}).limit(200))
    deleted = 0
    for t in to_del:
        uname = t.get("username")
        try:
            if pixels_col is not None:
                pixels_col.delete_many({"username": uname})
            if chats_col is not None:
                chats_col.delete_many({"username": uname})
            if public_tpl_col is not None:
                public_tpl_col.delete_many({"author": uname})
            users_col.delete_one({"_id": t["_id"]})
            deleted += 1
        except Exception as e:
            print("cleanup err", uname, e)
    global _pixel_cache_ready
    if deleted:
        _pixel_cache_ready = False
    return {"ok": True, "deleted": deleted, "checked": len(to_del)}




# ---------- MAP SNAPSHOTS (mod backup, max 20) ----------
map_snapshots_col = None

def _ensure_snap_col():
    global map_snapshots_col
    if db is None:
        return None
    if map_snapshots_col is None:
        map_snapshots_col = db["map_snapshots"]
        try:
            map_snapshots_col.create_index("created", DESCENDING)
        except Exception:
            pass
    return map_snapshots_col

def _snapshot_from_cache() -> dict:
    ensure_pixel_cache()
    xs, ys, cs = [], [], []
    if _pixel_cache:
        for k, col in _pixel_cache.items():
            try:
                ...
                xs.append(int(a))
                ys.append(int(b))
                cs.append(color_to_wire(col) if "color_to_wire" in globals() else col)
            except Exception:
                pass
    return {"xs": xs, "ys": ys, "cs": cs, "count": len(xs)}

@app.get("/mod/map_snapshots")
async def list_map_snapshots(user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    col = _ensure_snap_col()
    if col is None:
        return []
    out = []
    for s in col.find({}, {"pixels": 0}).sort("created", DESCENDING).limit(20):
        out.append({
            "id": str(s["_id"]),
...
@app.post("/mod/map_snapshot")
async def create_map_snapshot(data: dict = Body(default={}), user=Depends(get_current_user)):
    """Save current map pixels. Max 20 snapshots (oldest deleted)."""
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    col = _ensure_snap_col()
    if col is None:
        raise HTTPException(503, "DB not ready")
    name = str((data or {}).get("name") or "").strip()[:40] or datetime.utcnow().strftime("map_%Y%m%d_%H%M")
    payload = await asyncio.to_thread(_snapshot_from_cache)
    if payload["count"] <= 0:
        raise HTTPException(400, "Map is empty")
    doc = {
        "name": name,
        "count": payload["count"],
        "by": user["username"],
    ...
    # keep only 20 newest
    ids = [x["_id"] for x in col.find({}, {"_id": 1}).sort("created", DESCENDING)]
    if len(ids) > 20:
        col.delete_many({"_id": {"$in": ids[20:]}})
    return {"ok": True, "id": str(res.inserted_id), "count": payload["count"], "name": name}

@app.get("/mod/map_snapshot/{sid}/download")
async def download_map_snapshot(sid: str, user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    col = _ensure_snap_col()
    if col is None:
    ...
    fname = (s.get("name") or "map").replace(" ", "_")[:40] + ".json"
    return Response(
        content=raw,
...
@app.post("/mod/map_snapshot/{sid}/restore")
async def restore_map_snapshot(sid: str, data: dict = Body(default={}), user=Depends(get_current_user)):
    """Replace current map with snapshot. Optional clear=true wipes existing first."""
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    col = _ensure_snap_col()
    if col is None or pixels_col is None:
        raise HTTPException(503)
    try:
        oid = ObjectId(sid)
    except Exception:
    ...
    if not s:
        raise HTTPException(404, "Not found")
    clear = bool((data or {}).get("clear", True))
    return await _restore_pixels_payload(s.get("xs") or [], s.get("ys") or [], s.get("cs") or [], user, clear)

@app.post("/mod/map_snapshot/upload")
async def upload_map_snapshot(data: dict = Body(...), user=Depends(get_current_user)):
    """Upload snapshot JSON: either store (save=true) and/or restore (restore=true)."""
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    xs = data.get("xs") or []
    ys = data.get("ys") or []
    cs = data.get("cs") or []
    if not xs or len(xs) != len(ys) or len(xs) != len(cs):
        raise HTTPException(400, "Invalid snapshot arrays")
    if len(xs) > 2_500_000:
        raise HTTPException(400, "Snapshot too large")
    do_save = bool(data.get("save", True))
    do_restore = bool(data.get("restore", False))
    clear = bool(data.get("clear", True))
    name = str(data.get("name") or "upload").strip()[:40]
    out = {"ok": True, "count": len(xs)}
    if do_save:
        col = _ensure_snap_col()
        if col is None:
            raise HTTPException(503)
        ...
        out["saved"] = True
    if do_restore:
        r = await _restore_pixels_payload(xs, ys, cs, user, clear)
        out["restored"] = r.get("restored", 0)
    return out

@app.delete("/mod/map_snapshot/{sid}")
async def delete_map_snapshot(sid: str, user=Depends(get_current_user)):
    if not user or not user.get("is_mod"):
        raise HTTPException(403, "Mod only")
    col = _ensure_snap_col()
    if col is None:
...
async def _restore_pixels_payload(xs, ys, cs, user, clear: bool):
    global _pixel_cache, _pixel_cache_ready, _pixel_arrays_dirty
    ...
    def _work():
        ...
        if clear:
            pixels_col.delete_many({})
        ops = []
        new_cache = {} if clear else dict(_pixel_cache or {})
        uname = user["username"]
        dname = user.get("display_name") or uname
        now = datetime.utcnow()
        for i in range(n):
            try:
                ...
                col = wire_to_color(cs[i]) if "wire_to_color" in globals() else str(cs[i])
                if not (0 <= x < CANVAS_SIZE and 0 <= y < CANVAS_SIZE):
                    continue
                ...
                ops.append(UpdateOne(
                    {"x": x, "y": y},
                    {"$set": {
                        "color": col, "username": uname, "display_name": dname,
                        "user_id": str(user["_id"]), "placed_at": now,
                    }},
                    upsert=True,
                ))
            except Exception:
        ...
        if ops:
            pixels_col.bulk_write(ops, ordered=False)
        _pixel_cache = new_cache
        _pixel_cache_ready = True
        _pixel_arrays_dirty = True
        return len(new_cache)

    restored = await asyncio.to_thread(_work)
    await manager.broadcast({"type": "clear_all"})
    # clients should reload — send tip
    await manager.broadcast({"type": "map_restored", "count": restored, "by": user["username"]})
...
async def health():
    try:
        ...
        if _rnd.random() < 0.08 and users_col is not None:
            asyncio.create_task(_do_cleanup_inactive())
    except Exception:
        pass
    base = os.path.dirname(os.path.abspath(__file__))
    cwd = os.getcwd()
